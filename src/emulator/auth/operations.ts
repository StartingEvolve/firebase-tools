...
function signUp(
    ...
    if (reqBody.password) {
        ...
        updates.passwordHash = hashPassword(reqBody.password, updates.salt, true);
    ...
    for (let index = 0; index < reqBody.users.length; index++) {
        ...
        try {
            ...
            if (userInfo.passwordHash) {
                ...
                fields.passwordHash = hashPassword(userInfo.rawPassword, fields.salt, true);
                fields.passwordUpdatedAt = uploadTime.getTime();
            }
...
function batchDelete(
    ...
    const errors: Required<Schemas["GoogleCloudIdentitytoolkitV1BatchDeleteAccountsResponse"]["errors"]> = [];
...
export function resetPassword(
    ...
    if (reqBody.newPassword) {
        ...
        const passwordHash = hashPassword(reqBody.newPassword, salt, true);
